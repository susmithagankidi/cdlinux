...
echo "$ASG"
aws cloudwatch put-metric-alarm --alarm-name "$ASG-alarm" --alarm-description "Alarm for New ASG" --namespace "AWS/EC2" --metric-name CPUUtilization --statistic Average --period 60 --evaluation-periods 3 --threshold 1 --comparison-operator GreaterThanOrEqualToThreshold --dimensions Name=AutoScalingGroupName,Value=CodeDeploy_asgmetric_d-9F67OA0O4 --alarm-actions arn:aws:sns:ap-south-1:272842187865:test-perm --region $REGION
